...
public :
	...
	void render_world_vbo(bool debug, bool doTransparent, int WorldShader, float deltaTimeCumul)
	{
		glDisable(GL_BLEND);
		YRenderer* Renderer = YRenderer::getInstance();
		...
		for (int x = 0; x < MAT_SIZE; x++)
		{
			for (int y = 0; y < MAT_SIZE; y++)
			{
				for (int z = 0; z < MAT_HEIGHT; z++)
				{
					...
					Renderer->sendMatricesToShader(WorldShader); //Envoie les matrices au shader

					Renderer->sendTimeToShader(deltaTimeCumul, WorldShader);
